...
import { ref } from 'vue';
import { DcgjFormPro } from '@/components';
import type { FormConfigProps } from '@/components';
...
5. **registerComponent**：可通过 `import { registerComponent } from '@/components'` 注册自定义控件类型。
6. **setRequestAdapter**：可通过 `import { setRequestAdapter } from '@/components'` 替换默认的请求适配器。
