...
if [ $? -ne 0 ]; then
  ...
  printf '{"continue": true, "hookSpecificOutput": {"hookEventName": "Stop", "additionalContext": "类型检查未通过，请修复：\n%s"}}\n' "$ERRORS"
  exit 0
fi

# === 2. 格式化兜底（补偿 apply_patch 绕过 PostToolUse） ===
CHANGED_FILES=$(git diff --name-only HEAD 2>/dev/null | grep -E '\.(ts|tsx|js|jsx|css|less|scss|json)$' || true)
if [ -n "$CHANGED_FILES" ]; then
  echo "$CHANGED_FILES" | xargs pnpm exec prettier --write 2>/dev/null || true
fi

# === 3. 危险区兜底扫描 ===
DANGEROUS_PATTERNS="src/shared/components/Base|src/router/|src/guards/|src/interceptors/|src/styles/global|src/styles/theme|src/config/|vite.config|tsconfig|\.env"
