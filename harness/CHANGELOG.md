...
## Unreleased — Figma / 截图上下文路由

- **按页面类型路由真实示例**：PC 模板约定先识别列表、新建/编辑或详情，再只加载同类型示例；示例目录使用 OVERVIEW 做特征路由，并明确示例不是字段、尺寸或操作的事实来源。*为什么*：一次加载跨页面业务示例会引入无关上下文，降低 Figma Frame 与产出结构的匹配度。
- **页面开发与技术方案增加设计证据**：`/new-page`、`/tech-solution` 和跨 agent 的 `AGENTS.md` 增加 Frame/node、页面类型、组件映射、视觉状态、滚动归属与多视口验收，同时保留 v2.9 的拆分预判。*为什么*：Token 与画布只能约束基础视觉，无法保证页面结构和组件状态贴合设计。
- **组件能力改为版本化取证**：PC 初始化从 lockfile、组件库导出入口和项目真实调用生成组件索引，不在通用模板中假设某个组件或默认行为一定存在。*为什么*：dcgj-ui 持续新增组件，硬编码旧清单会让 agent 重复造轮子或调用当前版本不存在的 API。
