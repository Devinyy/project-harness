...
> 初始化 Harness 时必须先填写本节。未填写前，下面的组件目录结构只能视为示例。

- 是否适用：`是 / 否 / 部分适用`
- 当前项目组件语法：`<React TSX / Vue SFC / 其他>`
- 真实组件目录：
  - `<目录 1>`：`<职责>`
  - `<目录 2>`：`<职责>`
- 不要使用的目录或写法：
  - `<例如 hooks / composables / shared/ui / components/base 中不适用的项>`
- 与模板不同的地方：
  - `<请填写>`
- 参考示例：
  - React 项目：`docs/examples/react/EXAMPLE_COMPONENT_PATTERN_REACT.md`
  - Vue 项目：`docs/examples/vue/EXAMPLE_COMPONENT_PATTERN_VUE.md`
...
- [ ] 本文档中的组件目录在真实项目中存在
- [ ] 本文档中的组件语法与当前项目技术栈一致
- [ ] 本文档中的 React / Vue 示例已按项目实际收敛
- [ ] 本文档中的 shared / feature / page 边界能在真实代码中找到样例
- [ ] 本文档最后校验日期已更新
