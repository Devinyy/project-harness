...
> 初始化 Harness 时必须先填写本节。未填写前，下面的 service / adapter / store 结构只能视为示例。
...
- 真实 Mock 方案：`<请填写>`
- 不要使用的目录或写法：
  - `<例如 services/、api/、adapter/、query layer 中不适用的项>`
- 与模板不同的地方：
  - `<请填写>`
...
- [ ] 本文档中的 service 目录在真实项目中存在
- [ ] 本文档中的请求封装入口与当前代码一致
- [ ] 本文档中的 API 约定与接口文档或当前实现一致
- [ ] 本文档中的错误码和分页规则已被真实接口验证
- [ ] 本文档最后校验日期已更新
